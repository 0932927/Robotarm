...
void onestep(){
  ...
  if( (hoekverschil() > 0.8) && (linksRechts == 1 )){
  //rechtsom
  Serial.print("hoekverschil: ");
        Serial.println(hoekverschil());
    switch(y){
      case 0:
        write(1,0,0,0);
        y++;
  ...
  }else if( (hoekverschil() > 0.8) && (linksRechts == 0 )){
  //linksom
    switch(x){
      case 0:
        write(0,0,0,1);
...
float hoekverschil(){
  if(stepperangle < angle){
    linksRechts = 1; // true is links false is rechts
    //Serial.println("hoi");
    return angle - stepperangle ;
    
  }else if(stepperangle > angle){
    //Serial.println("doie");
    linksRechts = 0;
    return stepperangle - angle;  
  } else if(stepperangle == angle){
    linksRechts = 2;
  }
}
